# Healthcare Claims Management Web Application

A full-stack CMS-1500 healthcare insurance claims management system.
Built with **React 18 + Vite** (frontend), **FastAPI + Python 3.11+** (backend), and **MS-SQL Server** (database).

Developed with the [PropelIQ](https://propeliq.ai) AI-assisted development framework.
...
| **MS-SQL Server Express** | 2019+ | [Microsoft download](https://www.microsoft.com/en-us/sql-server/sql-server-downloads) |
| **ODBC Driver 17 for SQL Server** | 17+ | [Microsoft docs](https://docs.microsoft.com/en-us/sql/connect/odbc/download-odbc-driver-for-sql-server) — required by pyodbc |
...
Open `.env` and update the required values (see [Environment Variables](#5-environment-variables) section below).
...
Open **SSMS** or use `sqlcmd` to execute the scripts **in order**:
...
sqlcmd -S localhost\sqlexpress -E -i database\03_create_indexes.sql
sqlcmd -S localhost\sqlexpress -E -i database\04_stored_procedures.sql
sqlcmd -S localhost\sqlexpress -E -i database\05_seed_data.sql
```

**Verify the setup:**
```bash
sqlcmd -S localhost\sqlexpress -E -d HealthCareDB -Q "EXEC sp_Login 'admin','admin'"
# Expected: 1 row with Username=admin, Role=Admin
```

**Default credentials (non-production only):** `admin` / `admin`
...
3. You should see the **Claims List** page
...
pytest tests/integration/ -v               # integration tests (requires live DB)
```
...
|---|---|---|---|
| `DATABASE_URL` | ✅ | — | Full pyodbc connection string |
| `JWT_SECRET_KEY` | ✅ | — | 256-bit hex: `openssl rand -hex 32` |
...
| `AZURE_ENDPOINT` | ⬜ | — | Required if `LLM_PROVIDER=azure` |
| `AZURE_API_KEY` | ⬜ | — | Azure OpenAI API key |
| `AZURE_DEPLOYMENT` | ⬜ | — | Azure deployment name |
| `CHROMA_PERSIST_DIR` | ⬜ | `../ai_data/chroma` | ChromaDB persistence path |
| `VITE_API_BASE_URL` | ✅ (FE) | `http://localhost:8000` | Backend base URL for axios |

> ⬜ = optional — AI features gracefully return 503 when not configured
...
├── database/          # SQL scripts (DDL, SPs, seed data)
├── uploads/           # Document file storage (git-ignored)
├── ai_data/           # Vector store (git-ignored)
├── .env.example       # Environment variable template
└── .propel/           # PropelIQ framework artifacts
    └── context/       # spec.md, design.md, epics.md, stories.md, task-plan.md
```
...
- **Frontend:** React 18 + Vite + TypeScript + antd + React Hook Form + Zustand + Recharts
- **Backend:** FastAPI + Python 3.11 + pyodbc (stored procedures only)
- **Database:** MS-SQL Server with 8 tables, 15 stored procedures
- **AI (Phase 2):** LangChain + LangGraph + ChromaDB (RAG)

See [.propel/context/design.md](.propel/context/design.md) for full architecture documentation.

---

## 11. PropelIQ Framework

This project was developed using the **PropelIQ AI-assisted development framework**.

To use PropelIQ commands (e.g., `/create-spec`, `/implement-tasks`):
1. Ensure your `.env` contains a valid `CONTEXT7_API_KEY`
2. Open VS Code and press `Ctrl+Alt+I` to open the AI chat
3. Type `/` to see available PropelIQ commands

See all PropelIQ workflows in `.propel/prompts/`.
